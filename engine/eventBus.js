...
import { ENABLE_ENGINE_PROFILE, EVENTBUS_QUEUE_YIELD } from '../data/config.js';

/**
 * Copy args for audit history: fast path for primitives; deep clone when objects/functions appear.
 */
function cloneArgsForHistory(args) {
  ...
  if (!needsDeep) return args.slice();
  try {
...
class EventBus {
  constructor(options = {}) {
    ...
    /** Cumulative ms spent in history arg cloning per profiling window (see resetEngineProfileCounters). */
    this._profileHistoryCloneMs = 0;
  }
  ...
  _logEvent(event, args) {
    ...
    if (ENABLE_ENGINE_PROFILE) {
      ...
      deepArgs = cloneArgsForHistory(args);
      this._profileHistoryCloneMs += performance.now() - t0;
    } else {
      deepArgs = cloneArgsForHistory(args);
    }
  ...
  async processQueues() {
    ...
    try {
      ...
      // --- 3. Optional yield so async listeners can run (skip when EVENTBUS_QUEUE_YIELD=false) ---
      if (EVENTBUS_QUEUE_YIELD && eventsProcessed > 0) {
  ...
  resetEngineProfileCounters() {
    this._profileHistoryCloneMs = 0;
  }

  getEngineProfileSnapshot() {
    return {
      historyCloneMs: this._profileHistoryCloneMs,
      lastProcessDuration: this._performanceMetrics.lastProcessDuration,
    };
  }
