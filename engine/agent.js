...
class Agent {
  ...
  constructor(data) {
    ...
    if (!this.job) {
        if (this.lifeStage === 'child') {
            this.job = { title: 'Student', salary: 0, hours: [9, 17] };
    ...
    const isLaborPool =
      ...
      this.job?.title &&
      this.job.title !== 'Unemployed' &&
      this.job.title !== 'Student' &&
      this.job.title !== 'Retired';
    if (isLaborPool) {
        const assignedWork = assignWorkLocationByJob(this, worldGraph);
        if (assignedWork) this.workLocationId = assignedWork;
    }
    ...
    this.homelessSinceTick = Number.isFinite(homelessSinceTick) ? homelessSinceTick : null;
  ...
  serialize() {
    ...
    return {
      ...
      homelessSinceTick: sanitize(this.homelessSinceTick),
