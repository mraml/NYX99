import {
  ...
  DB_PATH,
  INITIAL_AGENTS,
  ENABLE_ENGINE_PROFILE,
  ENABLE_DEPARTURE_SANITIZER,
  DEPARTURE_SANITIZER_INTERVAL_TICKS,
  ENABLE_ROUTINE_DIAGNOSTICS,
  ROUTINE_DIAGNOSTICS_INTERVAL_TICKS,
} from '../data/config.js';
...
import { clearActivityCache } from './agent/agentActivity.js';
...
class Matrix {
  constructor() {
    ...
    this._profileSerializeMs = 0;
    /** locationId -> peer summaries for same-tick perception (main thread). */
    this.locationPeers = {};
  }

  _logRoutineDiagnostics(agents, worldTime) {
    if (!agents || agents.length === 0) return;
    const h = worldTime.getHours();
    let working = 0;
    const startHist = new Array(24).fill(0);
    const stateCounts = new Map();
    for (const a of agents) {
      const st = a.state || 'unknown';
      stateCounts.set(st, (stateCounts.get(st) || 0) + 1);
      if ((st || '').startsWith('fsm_working')) working++;
      const ws = Math.floor(Number.isFinite(a.workStartHour) ? a.workStartHour : 9);
      if (ws >= 0 && ws < 24) startHist[ws]++;
    }
    const top = [...stateCounts.entries()].sort((x, y) => y[1] - x[1]).slice(0, 10);
    logger.info(
      `[RoutineDiag] tick=${this.tickCount} simHour=${h} working=${working}/${agents.length} ` +
        `topStates=${JSON.stringify(top)} workStartHist=[${startHist.join(',')}]`
    );
  }

  _buildLocationPeersMap(allAgents) {
    const peers = {};
    for (const a of allAgents) {
      const loc = a.locationId || a.homeLocationId;
      if (!loc) continue;
      if (!peers[loc]) peers[loc] = [];
      peers[loc].push({
        id: a.id,
        name: a.name,
        state: a.state,
        mood: a.mood,
        stress: a.stress,
        inConversation: a.state === 'fsm_socializing' && !!a.stateContext?.conversationPartner,
        persona: a.persona,
        job: a.job,
        currentActivityName: a.currentActivityName,
  ...
  async tick() {
    ...
    try {
      ...
      if (ENABLE_ENGINE_PROFILE) {
        this.eventBus.resetEngineProfileCounters();
        this._profileSerializeMs = 0;
      }
      ...
      if (ENABLE_DEPARTURE_SANITIZER && this.tickCount % DEPARTURE_SANITIZER_INTERVAL_TICKS === 0) {
        const pruned = this.cacheManager.sanitizeDanglingAgentReferences();
        if (pruned > 0) {
          logger.info(`[Matrix] Departure sanitizer pruned ${pruned} dangling reference(s).`);
        }
      }

      this.locationPeers = this._buildLocationPeersMap(allAgents);
      ...
      if (ENABLE_ROUTINE_DIAGNOSTICS && this.tickCount % ROUTINE_DIAGNOSTICS_INTERVAL_TICKS === 0) {
        this._logRoutineDiagnostics(this.cacheManager.getAllAgents(), this.worldTime);
      }
      ...
      if (ENABLE_ENGINE_PROFILE && this.tickCount % 500 === 0) {
        const prof = this.eventBus.getEngineProfileSnapshot();
        logger.info(
          `[EngineProfile] tick=${this.tickCount} historyCloneMs=${prof.historyCloneMs.toFixed(2)} workerSerializeMs=${this._profileSerializeMs.toFixed(2)} lastQueueProcessMs=${prof.lastProcessDuration.toFixed(2)}`
        );
      }
    } finally {
  ...
  async _runAgentUpdatesInWorkers(allAgents) {
    ...
    const tSerialize = ENABLE_ENGINE_PROFILE ? performance.now() : 0;
    for (const agent of allAgents) {
    ...
    if (ENABLE_ENGINE_PROFILE) {
      this._profileSerializeMs = performance.now() - tSerialize;
    }
    ...
    const tickPayloadBase = {
      ...
      worldState: this.worldState,
      locationAgentCount: Object.fromEntries(this.locationAgentCount), // SERIALIZATION FIX: Convert Map to Object
      locationPeers: this.locationPeers || {},
