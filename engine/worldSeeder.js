...
function getAffordances(type) {
    ...
    switch (type) {
        ...
        case 'grocery_store': return [...base, { action: 'fsm_shopping' }, { action: 'fsm_working_service' }];
        case 'mall': return [...base, { action: 'fsm_shopping' }, { action: 'fsm_socializing' }, { action: 'fsm_recreation' }, { action: 'fsm_working_service' }];
        case 'record_store': return [...base, { action: 'fsm_shopping' }, { action: 'fsm_recreation' }, { action: 'fsm_working_service' }];
        case 'comic_shop': return [...base, { action: 'fsm_shopping' }, { action: 'fsm_recreation' }, { action: 'fsm_socializing' }, { action: 'fsm_working_service' }];
        case 'bookstore': return [...base, { action: 'fsm_shopping' }, { action: 'fsm_recreation' }, { action: 'fsm_working_service' }];
        case 'video_store': return [...base, { action: 'fsm_shopping' }, { action: 'fsm_recreation' }, { action: 'fsm_working_service' }];
        case 'movie_theater': return [...base, { action: 'fsm_recreation' }, { action: 'fsm_socializing' }, { action: 'fsm_working_service' }];
        case 'coffee_shop': return [...base, { action: 'fsm_eating' }, { action: 'fsm_socializing' }, { action: 'fsm_recreation' }, { action: 'fsm_working_service' }];
        case 'library': return [...base, { action: 'fsm_recreation' }, { action: 'fsm_working_service' }];
...
export function hydrateWorldGraph(worldGraph) {
    ...
    nodeKeys.forEach(key => {
        ...
        if (node.type === 'home') {
        ...
        } else if (['store', 'grocery_store', 'mall', 'record_store', 'comic_shop', 'bookstore', 'video_store', 'bar', 'restaurant', 'office', 'movie_theater', 'coffee_shop', 'venue'].includes(node.type)) {
            businessCount++;
            ...
            else if (node.type === 'movie_theater') node.open_hours = [11, 1];
            else if (node.type === 'grocery_store') node.open_hours = [7, 23];
            else if (node.type === 'mall') node.open_hours = [10, 22];
            else if (node.type === 'coffee_shop') node.open_hours = [6, 21];
