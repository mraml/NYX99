...
const Actions = {
    ResolvePurpose: (agent) => {
        if (!agent.stateContext.shoppingPurpose) {
            const activeIntent = agent.intentionStack?.[agent.intentionStack.length - 1];
            agent.stateContext.shoppingPurpose = activeIntent?.shoppingPurpose || 'discretionary';
        }
    ...
    EvaluateBudget: (agent, context) => {
        ...
        let itemType = 'protein_bar';
        let itemCount = 5;
        const purpose = agent.stateContext.shoppingPurpose || 'discretionary';

        if (purpose === 'groceries') {
            strategy = 'stock_up';
            itemType = conscientiousness > 0.6 ? 'canned_soup' : 'protein_bar';
            itemCount = Math.max(4, GAME_BALANCE.COSTS.GROCERY_STOCK_TARGET || 8);
            multiplier = (GAME_BALANCE.COSTS.GROCERY_BULK_TRIP_MULTIPLIER || 1.25) * (conscientiousness > 0.6 ? 0.9 : 1.0);
        }

        // Retail Therapy / Impulse Buying
        if (purpose !== 'groceries' && (stress > 60 || conscientiousness < 0.3)) {
            multiplier = 1.5; 
            strategy = 'impulse';
            itemType = 'energy_drink'; // Or random premium item
        } 
        // Frugal Shopping
        else if (purpose !== 'groceries' && conscientiousness > 0.7) {
            multiplier = 0.8; 
            strategy = 'frugal';
        ...
        agent.stateContext.plan = {
            totalCost: (purpose === 'groceries' ? (GAME_BALANCE.COSTS.GROCERIES || 15) : baseCost) * multiplier,
            strategy: strategy,
            itemType: itemType,
            itemCount: itemCount,
            purpose
    ...
    HandleSuccess: (agent, context) => {
        ...
        const purpose = agent.stateContext.plan.purpose || 'discretionary';
        ...
        } else if (purpose === 'groceries') {
            agent.mood = Math.min(100, (agent.mood ?? 0) + 1);
            agent.stress = Math.max(0, (agent.stress ?? 0) - 2);
            agent.stateContext.lastGroceryTick = context.worldState.currentTick;
            eventBus.emitNow('db:writeMemory', 'low', agent.id, context.worldState.currentTick, `Did a grocery run and stocked up for the week.`);
        }
...
const ShoppingTree = new Sequence([
    new Action(Actions.ResolvePurpose),
