...
const BUSY_STATES_NO_CHAT = new Set(['fsm_sleeping', 'fsm_commuting', 'fsm_working']);
...
const Actions = {
    ...
    FindNewPartner: (agent, context) => {
        ...
        const candidates = agentsHere.filter(a => {
            ...
            if ((a.state || '').startsWith('fsm_working')) return false;
            return !BUSY_STATES_NO_CHAT.has(a.state);
        });

        if (candidates.length > 0) {
            const scored = candidates.map((c) => {
                const rel = agent.getRelationship(c.id);
                const aff = rel.affinity ?? 0;
                const recencyPenalty = agent.lastSocialPartner === c.id ? -18 : 0;
                const jitter = (Math.random() - 0.5) * 10;
                return { c, score: aff + recencyPenalty + jitter };
            });
            scored.sort((a, b) => b.score - a.score);
            const partner = scored[0].c;
