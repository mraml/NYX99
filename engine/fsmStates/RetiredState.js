...
import { transitionWithCommuteIfNeeded } from '../agentUtilities.js';
...
const Actions = {
    ...
    GoEat: (agent) => {
        return transitionWithCommuteIfNeeded(agent, 'fsm_eating', { reason: 'retired_hungry' });
    },

    GoSocialize: (agent) => {
        return transitionWithCommuteIfNeeded(agent, 'fsm_socializing', { reason: 'retired_lonely' });
    },

    GoRecreation: (agent) => {
        return transitionWithCommuteIfNeeded(agent, 'fsm_recreation', { reason: 'retired_bored' });
    },
    ...
    DoRetiredActivity: (agent) => {
        ...
        // Rotate activity roughly once every 20 ticks (~5 sim minutes)
        if (Math.random() < 0.05) {
            const hour = agent.matrix?.worldTime?.getHours() || 12;
            updateCurrentActivity(agent, 'fsm_retired', hour);
        }
