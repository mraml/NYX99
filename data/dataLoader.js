...
/** Fixed calendar dates (month/day) from events.yaml */
export const CALENDAR_EVENTS = [];
/** Weighted ambient lines when no active world_events (dashboard). */
export const AMBIENT_CITY_STATUS = [];
/** City-wide random incidents (spawned in worldService). */
export const WORLD_RANDOM_EVENTS = [];
// --- END ---
...
class DataLoader {
  ...
  validateActivityLocationTypes() {
    const activityRows = this.activities?.activities;
    const graphNodes = this.locationGraph?.nodes;
    if (!Array.isArray(activityRows) || !Array.isArray(graphNodes)) return;

    const knownTypes = new Set(graphNodes.map((n) => n.type).filter(Boolean));
    knownTypes.add('any');
    const unknownByType = new Map();

    for (const activity of activityRows) {
      const locTypes = Array.isArray(activity.location_types) ? activity.location_types : [];
      for (const locType of locTypes) {
        if (!knownTypes.has(locType)) {
          if (!unknownByType.has(locType)) unknownByType.set(locType, []);
          unknownByType.get(locType).push(activity.name);
        }
      }
    }

    if (unknownByType.size === 0) return;
    for (const [locType, names] of unknownByType.entries()) {
      const examples = names.slice(0, 5).join(', ');
      this.log(`[DataLoader] ⚠️ Unknown activity location_type "${locType}" used by ${names.length} activities (e.g. ${examples}).`, 'medium');
    }
  }
  ...
  _processWorldData(worldData, eventsData) {
    ...
    if (eventsData) {
        ...
        if (Array.isArray(eventsData.calendar_events)) {
            CALENDAR_EVENTS.push(...eventsData.calendar_events);
        }

        AMBIENT_CITY_STATUS.length = 0;
        if (Array.isArray(eventsData.ambient_city_status)) {
            AMBIENT_CITY_STATUS.push(...eventsData.ambient_city_status);
        }

        WORLD_RANDOM_EVENTS.length = 0;
        if (Array.isArray(eventsData.world_random_events)) {
            WORLD_RANDOM_EVENTS.push(...eventsData.world_random_events);
        }
    }
  ...
  async loadAllData() {
    ...
    this.validateActivityLocationTypes();
