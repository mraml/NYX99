...
const RETAIL_LOCATION_TYPES = new Set(['store', 'grocery_store', 'mall', 'comic_shop', 'record_store', 'bookstore', 'video_store']);
const BUSINESS_LOCATION_TYPES = new Set([
  'office', 'bar', 'restaurant', 'store', 'grocery_store', 'mall',
  'comic_shop', 'record_store', 'bookstore', 'video_store',
  'venue', 'gym', 'movie_theater', 'coffee_shop'
]);
...
class WorldGraph {
  ...
  _loadGraph() {
    ...
    for (const node of graphData.nodes) {
      ...
      // Business logic
      if (BUSINESS_LOCATION_TYPES.has(node.type)) {
        ...
        node.employee_ids = [];
        if (!node.operating_hours || !Array.isArray(node.operating_hours) || node.operating_hours.length < 2) {
          if (node.type === 'bar' || node.type === 'venue') node.operating_hours = [17, 2];
          else if (node.type === 'movie_theater') node.operating_hours = [11, 1];
          else if (node.type === 'office') node.operating_hours = [8, 18];
          else if (node.type === 'grocery_store') node.operating_hours = [7, 23];
          else if (node.type === 'mall') node.operating_hours = [10, 22];
          else if (node.type === 'coffee_shop') node.operating_hours = [6, 21];
          else node.operating_hours = [9, 21];
        }
      }
      ...
      if (!node.operating_hours) node.operating_hours = [0, 24];
  ...
  _generateBaseAffordances(type) {
      switch (type) {
        ...
        case 'grocery_store': return [
            { action: 'fsm_shopping', quality: 1.2 },
            { action: 'fsm_eating', quality: 0.2 }
        ];
        case 'mall': return [
            { action: 'fsm_shopping', quality: 1.3 },
            { action: 'fsm_recreation', quality: 0.8 },
            { action: 'fsm_socializing', quality: 0.7 }
        ];
        case 'comic_shop': return [
            { action: 'fsm_shopping', quality: 1.1 },
            { action: 'fsm_recreation', quality: 0.7 },
            { action: 'fsm_socializing', quality: 0.6 }
        ];
        case 'record_store': return [
            { action: 'fsm_shopping', quality: 1.1 },
            { action: 'fsm_recreation', quality: 0.8 }
        ];
        case 'bookstore': return [
            { action: 'fsm_shopping', quality: 0.9 },
            { action: 'fsm_recreation', quality: 0.9 },
            { action: 'fsm_socializing', quality: 0.4 }
        ];
        case 'video_store': return [
            { action: 'fsm_shopping', quality: 0.8 },
            { action: 'fsm_recreation', quality: 0.7 }
        ];
        case 'movie_theater': return [
            { action: 'fsm_recreation', quality: 1.4 },
            { action: 'fsm_socializing', quality: 0.7 }
        ];
        case 'coffee_shop': return [
            { action: 'fsm_eating', quality: 0.7 },
            { action: 'fsm_socializing', quality: 0.9 },
  ...
  getDynamicAffordances(node, currentHour, currentDay, worldEvents) {
    ...
    if (RETAIL_LOCATION_TYPES.has(node.type)) {
