...
export function processSocialInteractions(lod1Agents, worldNodes, eventBus, tickCount, worldState) {
  // Group socializing agents by location once (O(N)) to avoid repeated global scans
  const byLocation = new Map();
  for (const agent of lod1Agents) {
    if (agent.state !== 'fsm_socializing') continue;
  ...
  for (const [, group] of byLocation) {
    for (const agentA of group) {
      if (processed.has(agentA.id)) continue;

      // Candidates are only agents in the same location group - no global scan
      const potentialPartners = group.filter(p =>
        !processed.has(p.id) && p.id !== agentA.id
      );
