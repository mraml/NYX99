...
export async function updateAgent(agent, matrix, hour) {
  ...
  if (matrix.locationPeers && agent.locationId) {
    const list = matrix.locationPeers[agent.locationId] || [];
    agent.perceivedAgents = list.filter((p) => p.id !== agent.id);
  } else {
    agent.perceivedAgents = [];
  }
